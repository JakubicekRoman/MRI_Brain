...
class Config:   
    ...
    # scales = np.array([np.sqrt(4), 1])
    scales = np.array([1])
    sigmas = scales - 1
    init_lr = 0.0001  #fungujepro maskuN
    resize_factor = 1
    iterations = [20,30,35]
    gamma = 0.1  
    num_batches = 5
...
data_path = r'C:\Data\MRI_Glioms\MRI_Brain\Bogner\Outputs'
# seq = 'Bogner_DCE'   
seq = 'Bogner_DSC'
...
file_name = glob(data_path + os.sep + seq + '.nii.gz',recursive=True)


# for file_num, file_name in enumerate(file_names):
    
img = nib.load(file_name[0])
imgs = img.get_fdata()  
imgs = imgs.astype(config.np_dtype)
    
# imgs = imgs[::2,::2,::2,:]
imgs = util.crop_center_3D_batch(imgs,120,120,120)
...
masks = np.ones(imgs.shape).astype(config.np_dtype)
# masks = (imgs>0.01).astype(config.np_dtype)
# plt.imshow(masks[0,70,:,:])
# plt.show()
...
for scale_num, (scale, sigma) in enumerate(zip(config.scales,config.sigmas)):
    ...
    # imgs_res, masks_res = util.resize_pyramid(imgs, masks, scale, sigma, config.np_dtype)
    imgs_res = imgs.copy()
    # masks_res = masks.copy()
    # imgs_res = imgs
    ...
    m, s = torch.nanmean(imgs_res,(2,3,4)), torch.std(imgs_res,(2,3,4))
    ...
    with util.Timer():
        for it in range(config.iterations[-1]):
            ...
            for batch_num, inds in enumerate(randomizer):
                ...
                imgs_res_batch = imgs_res[inds,:,:,:].to(config.device).to(config.torch_dtype)
                # masks_res_batch = masks_res[inds,:,:,:].to(config.device).to(config.torch_dtype)
                ...
                out_masks = torch.ones(output_batch.shape).to(config.torch_dtype).to(config.device)
...
del imgs, grid, imgs_res_batch, masks, out_masks, output, output_batch, imgs_res

imgs = img.get_fdata()  
imgs = imgs.astype(config.np_dtype) 
# imgs = util.crop_center_3D_batch(imgs,120,120,120)
imgs = np.transpose(imgs,[3,2,0,1])
imgs = torch.unsqueeze(torch.from_numpy(imgs),1)

output = []
Theta = []
randomizer = util.Randomizer(imgs.shape[0], imgs.shape[0])
for batch_num, inds in enumerate(randomizer):
    imgs_batch = imgs[inds,:,:,:].to(config.device).to(config.torch_dtype)
    theta = construct_transf_matrix_3D(params, config.resize_factor, inds, config.device)
    
    grid = F.affine_grid(theta[:,0:3,:], imgs_batch.shape, align_corners=config.align_corners)
    output_batch = F.grid_sample(imgs_batch, grid, padding_mode="zeros", align_corners=config.align_corners, mode=config.interp_mode)
    output.append(output_batch[:,0,:,:].detach().cpu().numpy())
 

output = np.concatenate(output,0)
output= randomizer.order_corection(output)
# output = (( output*s[:,None, None].cpu().detach().numpy())+(m[:,None, None]).cpu().detach().numpy()).astype(np.int16)
output = np.transpose(output,[2,3,1,0])
output = output.astype(np.int16)

res = nib.Nifti1Image(output, img.affine, img.header)
res.get_data_dtype() == np.dtype(np.int16)
nib.save(res, os.path.join(data_path, save_sufix+'.nii.gz'))  
